...
// interface for post to get it from api having these types
...
// receving post that is rednered
interface PostProps {
    post: Post;
...
export function Post({ post, setPosts }: PostProps) {
    ...
    useEffect(() => {
        ...
        // 🔽 NEW: load comments for this post when the component mounts
    useEffect(() => {
                    ...
                    return {
                        ...
                        comments: p.comments?.map((comment) => {
                            if (comment.id === id) {
                                return {
                                    ...comment,
                                    content, updatedDate: new Date().toISOString(),
                                };
                            }
                            return comment;
                        }),
    ...
    const postComment = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setPosts((prev) => prev.map((p) => {
                ...
                {
                    return {
                        ...p, comments: p.comments ? [data, ...p.comments] : [data],
                    };
                }
                return p;
            }));
            ...
            <div className={classes.top}>
                <div className={classes.author}>
                    <button onClick={() => {
                        ...
                        <img className={classes.avatar} src={post.author.profilePicture || "/user1.jpeg"} alt="" />
                    </button>
                    <div>
                        ...
                        <div className={classes.date}>
                            {timeAgo(new Date(post.updateDate || post.creationDate))}
                            {post.updateDate ? " . Edited " : ""}
                        </div>
                    </div>
                </div>
                <div>
