package com.hivenet.features.notifications.service;
...
import com.hivenet.features.notifications.model.NotificationType;
import com.hivenet.features.notifications.repository.NotificationRepository;
...
public class NotificationService {
    ...
    public void sendLikeNotification(AuthenticationUser author, AuthenticationUser recipient, Long resourceId) {
        if (author.getId().equals(recipient.getId())) {
            return;  // agar user khud ki post like karega to notification nahi jayega 
        }
        ...
        notificationRepository.save(notification); // saari details save karke 

        messagingTemplate.convertAndSend("/topic/users/" + recipient.getId() + "/notifications", notification); //ispe jayengi details /topic/users jo ki apne header se subscribed hai !
    }
